...
const contractAddress = '0x772Bb4fE2eCc9a3d2471457E799f62a6c3cDEaB7'
...
export default function Page() {
  ...
  const handleSetSellValue = event => {
...
const [aumValue, setAumValue] = useState(0);
  ...
  const handleSell = async () => {
    try {
      ...
      const signer = provider.getSigner();
      const smartContract = new ethers.Contract(contractAddress, abi, signer);
      const sellValueInWei = parseUnits(sellValue.toString(), "ether");
      const tx = await smartContract.sell(sellValueInWei.toString());
      console.log("Transaction hash:", tx.hash);
    } catch (err) {
      console.log(err);
    }
...
<Box sx={{ flexGrow: 1 }}>
      ...
      {isActive && (
        <Container maxWidth="sm">
          <Card sx={{ minWidth: 275, backgroundColor: '#01150', height: '550px', boxShadow: '5px 5px 10px #888888' }}>
            <CardContent>
              <Stack spacing={2}>
              ...
              <TextField
                ...
                label="Enter amount of Ether you want to buy ADK Token"
                defaultValue=""
                type="number"
              ...
              required
              id="outlined-required"
              label="Enter amount of ADK Token you want to sell"
              defaultValue=""
              type="number"
               onChange={handleSetSellValue}
              />
